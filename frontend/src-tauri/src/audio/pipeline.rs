...
use super::echo_cancel::AcousticEchoCanceller;
...
/// Mix mic + system audio after cancelling speaker echo from the microphone.
...
impl ProfessionalAudioMixer {
    fn new(sample_rate: u32) -> Self {
        Self {
            aec: AcousticEchoCanceller::new(sample_rate),
        }
    }

    fn mix_window(&mut self, mic_window: &[f32], sys_window: &[f32]) -> Vec<f32> {
        let cleaned_mic = self.aec.process(mic_window, sys_window);
        let max_len = cleaned_mic.len().max(sys_window.len());
        let mut mixed = Vec::with_capacity(max_len);

        for i in 0..max_len {
            let mic = cleaned_mic.get(i).copied().unwrap_or(0.0);
